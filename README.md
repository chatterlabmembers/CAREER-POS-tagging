...
4. Use the [CAREER POS noun/verb tagging coding guide](https://uchicago.app.box.com/file/2461112515106) to guide your coding decisions in `is_correct_tag` and `corrected_tag` columns. **Always double check your spelling!**

   - If you suspect transcription errors, please flag the file and report it to the Lab Manager.

5. Work through the entire .xlsx file. When finished, go back to the [CAREER POS tagging file tracker](https://uchicago.app.box.com/file/2442561906632) spreadsheet and add today's date to `Last date worked on` column. Add comments if needed.
...
3. For each utterance in the .xlsx file:
   ...
   - Provide the appropriate **tag** in the corresponding `tag` column. The tags that we are using in the current workflow are 1. noun (n), 2. proper noun (n:prop), 3. plural noun (n:pt), 4. letter noun (n:let), 5. verb (v), 6. auxiliary verb (aux), and 7. modal verb (mod). See [CLAN morphology tags](https://uchicago.app.box.com/file/1781179507108) for the full list of morphological tags.
     - For some tricky cases of noun and verb tags, see [CAREER POS noun/verb tagging coding guide](https://uchicago.app.box.com/file/2461112515106) for examples.
