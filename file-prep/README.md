...
4.  Once CLAN finishes processing all .cha files, it outputs an .ulx.cex file that documents all tokens the `mor @` function did not recognize.
    - NOTE: Most of the times, the tokens that go unrecognized are 1) any incomplete speech indicated by hyphens, 2) noun phrases connected via underscores, 3) select compound nouns, and 4) any misspelled tokens. These unrecognized tokens' tag shows up as a question mark — you may want to do a systematic screening of those tokens.
...
## part 4: .cha to .xlsx conversion w extracted info :)

1.  TO BE UPDATED \>\>\>\>
2.  NEED an updated script to add in some of the fixes proposed for better tagging accuracy.
3.  should be ready for human validation after adding in the fixes.
